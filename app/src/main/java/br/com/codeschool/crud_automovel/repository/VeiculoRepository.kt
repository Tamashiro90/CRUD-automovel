...
class VeiculoRepository {
    ...
    suspend fun adicionaVeiculo(veiculo: Veiculo):String?{
        return try{
    ...
    suspend fun buscarVeiculos(consulta:String): List<Veiculo>{
        return try {
            val resultadosProprietario = colecaoVeiculos.whereGreaterThanOrEqualTo("nomeProprietario", consulta).whereLessThanOrEqualTo("nomeProprietario",consulta+"\uf8ff").get().await()
            val resultadosMarca = colecaoVeiculos.whereGreaterThanOrEqualTo("marca", consulta).whereLessThanOrEqualTo("marca",consulta+"\uf8ff").get().await()
            val resultadosModelo = colecaoVeiculos.whereGreaterThanOrEqualTo("modelo", consulta).whereLessThanOrEqualTo("modelo",consulta+"\uf8ff").get().await()
            val resulvadosPlaca = colecaoVeiculos.whereGreaterThanOrEqualTo("placa", consulta).whereLessThanOrEqualTo("placa",consulta+"\uf8ff").get().await()
            val mapaResultados = mutableMapOf<String, Veiculo>()
            listOf(resultadosProprietario,resultadosMarca,resultadosModelo,resulvadosPlaca).forEach{snapshot ->
